...
function getSumBetweenNumbers(n1, n2) {
  return Array.from({ length: n2 - n1 + 1 }, (_, i) => n1 + i).reduce((a, b) => a + b);
}
...
function isTriangle(a, b, c) {
  let res = false;
  if (a + b > c && a + c > b && c + b > a) res = true; return res;
}
...
function findFirstSingleChar(str) {
  const res = str.split('').filter((a) => str.lastIndexOf(a) === str.indexOf(a))[0];
...
function isCreditCardNumber(ccn) {
  ...
  const csum = parseInt(Array.from(str).at(-1), 10);
  const cardNumber = Array.from(str).reverse().slice(1);
  ...
  return (sum === csum) || (sum % 10 === 0 && csum === 0);
}
...
function getDigitalRoot(num) {
  ...
  const x = arr.reduce((prev, el) => Number(prev) + Number(el));
  if (x > 9) {
    arr = String(x).split('');
    const y = arr.reduce((prev, el) => Number(prev) + Number(el));
    return y;
  }
  return x;
}
